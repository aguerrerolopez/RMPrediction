...
folds_path = "./data/RYC_10STRATIFIEDfolds_muestrascompensada_cpblee.pkl"
data_path = "./data/ryc_data_mediansample_only2-12_TIC.pkl"
store_path = "Results/mediana_10fold_rbf/RYC_full_linear_15views_CARB_prun"+str(hyper_parameters['sshiba']["pruning_crit"])+".pkl"
...
maldi = gm_data['maldi']
gen = gm_data['gen']
cols = ['AMOXI/CLAV .1', 'PIP/TAZO.1', 'CEFTAZIDIMA.1', 'CEFOTAXIMA.1', 'CEFEPIME.1', 'AZTREONAM.1', 'IMIPENEM.1', 'MEROPENEM.1', 'ERTAPENEM.1']
...
for f in range(len(folds["train"])):
    ...
    x1_tr, x1_val = gen.loc[folds["train"][f]], gen.loc[folds["val"][f]]
    x2_tr, x2_val = fen.loc[folds["train"][f]], fen.loc[folds["val"][f]]
    ...
    # # Genotype
    x1 = np.vstack((x1_tr, x1_val))
    # # Fenotype
    x2 = np.vstack(x2_tr.values)
    # Familias
