...
hyper_parameters = {
    "sshiba": {"prune": 1, "myKc": 100, "pruning_crit": 1e-3, "max_it": int(5000)}
}
...
store_path = (
    "Results/normalizados/HRC_mkl2kernels_linpike_normalizados_prun"
    + str(hyper_parameters["sshiba"]["pruning_crit"])
    + ".pkl"
)
    ...
    if kernel == "pike":
        ...
        for asig in range(x0.shape[0]):
            ...
            topf_signal = np.concatenate(
                (np.arange(2000, 12000).reshape(-1, 1), x0[asig, :].reshape(-1, 1)),
                axis=1,
            )
