...
import categorical_nn as categorical_nn


# import categorical_semi_nn as categorical_ss_nn
...
class SSHIBA(object):
    ...
    def fit(self, *args, **kwargs):
        ...
        for arg in args:
            ...
            if arg['method'] == 'reg':
                ...
                if self.n_max > self.n[m]:
                    self.X[m]['mean'] = np.vstack(
                        (self.X[m]['mean'], np.NaN * np.ones((self.n_max - self.n[m], self.d[m]))))
                    ...
                    if (self.X_init is None):
                        for d in np.arange(self.d[m]):
                            if np.sum(self.SS_mask[m][:, d]) == self.SS_mask[m].shape[0]:
                                self.X[m]['mean'][self.SS_mask[m][:, d], d] = np.random.normal(0.0, 1.0, np.sum(
                                    self.SS_mask[m][:, d]))
                            else:
                                self.X[m]['mean'][self.SS_mask[m][:, d], d] = np.random.normal(
                                    np.nanmean(self.X[m]['mean'][:, d], axis=0),
                                    np.nanstd(self.X[m]['mean'][:, d], axis=0), np.sum(self.SS_mask[m][:, d]))
                    else:
            ...
            # Categorical
                ...
                if np.sum(self.SS_mask[m]) > 0:
                    ...
                    self.X[m]['mean'][~self.SS_mask[m]] = label_binarize(self.t[m]['mean'][~self.SS_mask[m]],
                                                                         classes=np.arange(self.d[m])).astype(float)
                    self.X[m]['cov'][~self.SS_mask[m]] = 0
                    self.cat_nn[m] = categorical_ss_nn.CategoricalNN(d_x=kwargs['dim_x'], dim=kwargs['nnd'],
                                                                     t_raw=self.t[m]['mean'], store=kwargs['store'],
                                                                     SS=True, mask=self.SS_mask[m])
                else:
                    self.SS.append(False)
                    self.t[m]['mean'] = np.copy(self.t[m]['data'])
                    self.t[m]['cov'] = np.zeros((self.t[m]['mean'].shape))
                    self.X[m]['mean'] = np.random.normal(0.0, 1.0, self.n_max * self.d[m]).reshape(self.n_max,
                                                                                                   self.d[m])
                    self.X[m]['cov'] = np.abs(
                        np.random.normal(0.0, 1.0, self.n_max * self.d[m]).reshape(self.n_max, self.d[m]))
                    self.cat_nn[m] = categorical_nn.CategoricalNN(d_x=kwargs['dim_x'], dim=kwargs['nnd'],
                                                                  t_raw=self.t[m]['data'], store=kwargs['store'])
            ...
            # Multilabel
                ...
                if self.n_max > self.n[m]:
                    self.t[m]['data'] = np.vstack(
                        (self.t[m]['data'], np.NaN * np.ones((self.n_max - self.n[m], self.d[m]))))
                self.SS_mask[m] = np.isnan(self.t[m]['data'])
                ...
                self.X[m]['mean'][~self.SS_mask[m]] = np.log(
                    np.abs((self.t[m]['data'][~self.SS_mask[m]]).astype(float) - 0.05) / (
                                1 - np.abs((self.t[m]['data'][~self.SS_mask[m]]).astype(float) - 0.05)))
                self.X[m]['cov'][~self.SS_mask[m]] = 0
                for d in np.arange(self.d[m]):
                    self.X[m]['mean'][self.SS_mask[m][:, d], d] = np.random.normal(
                        np.nanmean(self.X[m]['mean'][~self.SS_mask[m][:, d], d], axis=0),
                        np.nanstd(self.X[m]['mean'][~self.SS_mask[m][:, d], d], axis=0), np.sum(self.SS_mask[m][:, d]))
                    self.X[m]['cov'][self.SS_mask[m][:, d], d] = np.abs(
                        np.random.normal(np.nanmean(self.X[m]['mean'][~self.SS_mask[m][:, d], d], axis=0),
                ...
                if np.sum(self.SS_mask[m]) > 0:
                    ...
                    for d in np.arange(self.d[m]):
                        self.t[m]['mean'][self.SS_mask[m][:, d], d] = np.random.normal(
                            np.nanmean(self.t[m]['mean'][:, d], axis=0), np.nanstd(self.t[m]['mean'][:, d], axis=0),
                            np.sum(self.SS_mask[m][:, d]))
                    ...
                    if any(nans_t != 0) and self.SS_sep:
                        a = 1 if np.sum(
                            nans_t != 0) != 0 else 0  # If there is no view without nan we keep the first in the original view
                    ...
                    if not (self.X_init is None):
                        if np.max(self.X_init[m]['mean']) == 1 and np.min(self.X_init[m]['mean']) == 0:
                            self.X[m]['mean'] = (2 * self.X_init[m]['mean'] - 1).astype(
                                float)  # We cast the variables to a float in case the values used
                        else:
                            self.X[m]['mean'] = self.X_init[m]['mean'].astype(
                                float)  # We cast the variable to a float in case the values used
        ...
        self.q_dist = Qdistribution(self.X, self.n, self.n_max, self.d, self.Kc, self.m, self.sparse, self.method,
                                    self.SS,
    ...
    def fit_iterate(self, max_iter=int(1e3), pruning_crit=1e-6, tol=1e-3, feat_crit=1e-6, perc=False, verbose=0,
                    Y_tst=[None],
                    X_tst=[None], X_tr=[None], HL=0, AUC=0, ACC=0, mse=0, R2=0, m_in=[0], m_out=1, steps=100, mc_it=1,
                    store=True, dim_x=None, nnd=None):
        ...
        for i in range(max_iter):
            ...
            self.update(Y_tst, X_tst, X_tr, HL, AUC, ACC, mse, R2, m_in, m_out, steps, mc_it, store=store)
            ...
            if verbose:
                verboseprint('\rIteration %d Lower Bound %.1f K %4d' % (len(self.L), self.L[-1], q.Kc), end='\r',
                             flush=True)
    ...
    def depruning(self, pruning_crit):
        ...
        q.Z['cov'] = np.vstack((np.hstack((q.Z['cov'], pruning_crit * np.ones((q.Kc, K_prune)))),
                                pruning_crit * np.ones((K_prune, self.Kc))))
        q.Z['prodT'] = np.vstack((np.hstack((q.Z['prodT'], pruning_crit * np.ones((q.Kc, K_prune)))),
                                  pruning_crit * np.ones((K_prune, self.Kc))))
        # Pruning W and alpha
        for m in np.arange(self.m):
            q.W[m]['mean'] = np.hstack((q.W[m]['mean'], pruning_crit * np.ones((self.d[m], K_prune))))
            q.W[m]['cov'] = np.vstack((np.hstack((q.W[m]['cov'], pruning_crit * np.ones((q.Kc, K_prune)))),
                                       pruning_crit * np.ones((K_prune, self.Kc))))
            q.W[m]['prodT'] = np.vstack((np.hstack((q.W[m]['prodT'], pruning_crit * np.ones((q.Kc, K_prune)))),
                                         pruning_crit * np.ones((K_prune, self.Kc))))
    ...
    def struct_data(self, X, method, sparse=0, V=None, kernel=None, sig=0, sparse_fs=0, center=1, mask=None, deg=None):
        ...
        __nndim : dictionary
            A ditionary with 3 entries each one is a list.
            Each entry name has to be: 'xga_mean', 'xga_cov' and 'tgx'
            Each element of the list represent:
                hidden dim 1, ... (nº of hiddens) ...          
        """
        if not (V is None):
            if (kernel is None):
                kernel = 'rbf'
    ...
    def compute_predictions(self, X_tst, m_in=[0], m_out=1, tr=0):
        ...
        if self.method[m_out] == 'reg':
            ...
            if self.SS[m_out]:
                ...
                Y_pred = self.cat_nn[m_out].predict_proba()
            else:
    ...
    def compute_HL(self, Y_tst, X_tst, m_in=[0], m_out=1):
        ...
        Y_pred = self.compute_predictions(X_tst, m_in=m_in, m_out=m_out)
        if self.method[m_out] == 'cat':
            x_aprox = self.X[m_out]['mean'] + np.random.normal() * np.sqrt(self.X[m_out]['cov'])
            Y_pred = self.cat_nn[m_out].predict(torch.from_numpy(x_aprox))
    ...
    def compute_ACC(self, Y_tst, X_tst, m_in=[0], m_out=1):
        ...
        if self.method[m_out] == 'cat':
            ...
            Y_pred = (np.ones((Y_pred.shape[0], self.d[m_out])) * np.unique(Y_tst['data']))[
                label_binarize(np.argmax(abs(Y_pred), axis=1), classes=np.arange(self.d[m_out])).astype(bool)]
    ...
    def update(self, Y_tst=[None], X_tst=[None], X_tr=[None], HL=0, AUC=0, ACC=0, mse=0, R2=0, m_in=[0], m_out=1,
        ...
        for m in np.arange(self.m):
            ...
            if self.method[m] == 'reg':
                if self.SS[m]:
                    ...
                    for n in np.arange(self.n_max):
                        self.X[m]['prodT'] += np.dot(self.X[m]['mean'][n, np.newaxis].T,
                                                     self.X[m]['mean'][n, np.newaxis]) + np.diag(self.X[m]['cov'][n, :])
                        self.X[m]['sumlogdet'] += \
                        np.linalg.slogdet(np.diag(self.X[m]['cov'][n, self.SS_mask[m][n, :]]))[1]
                else:
            ...
            # Categorical
                ...
                q.tau[m]['a'] = 1
                q.tau[m]['b'] = 1
                self.cat_nn[m].SGD_step(q.Z['mean'], q.W[m]['mean'], q.tau[m]['a'], steps, mc_it=mc_it)
                ...
                if self.SS[m]:
                    self.t[m]['mean'][self.SS_mask[m]] = t_pred[self.SS_mask[m]]
                    self.train_acc.append(accuracy_score(self.t[m]['data'][~self.SS_mask[m]], t_pred[~self.SS_mask[m]]))
                    print("Train Accuracy:" + str(self.train_acc[-1]))
                else:
                    self.train_acc.append(accuracy_score(self.t[m]['data'], t_pred))
                    print("Train Accuracy:" + str(self.train_acc[-1]))
        ...
        if self.sparse[m]:
            ...
            if self.area_mask[m] is not None:
                for f in np.arange(len(np.unique(self.area_mask[m]))):
                    ...
                    q.W[m]['mean'][d, :] = np.linalg.multi_dot(
                        [(self.X[m]['mean'][:, d] - q.b[m]['mean'][0, d]).T, q.Z['mean'], w_cov]) * q.tau_mean(m)
            ...
            if not np.any(np.isnan(w_cov)):
                ...
                q.W[m]['mean'] = q.tau_mean(m) * np.linalg.multi_dot(
                    [np.subtract(self.X[m]['mean'], q.b[m]['mean']).T, q.Z['mean'], q.W[m]['cov']])
                # E[W*W^T]
    ...
    def update_b(self, m):
        ...
        q.b[m]['mean'] = q.tau_mean(m) * np.dot(
            np.sum(np.subtract(self.X[m]['mean'], np.dot(q.Z['mean'], q.W[m]['mean'].T)), axis=0)[np.newaxis, :],
            q.b[m]['cov'])
        q.b[m]['prodT'] = np.sum(q.b[m]['mean'] ** 2) + self.d[m] * q.b[m]['cov'][
            0, 0]  # mean of a noncentral chi-squared distribution
    ...
    def update_tau(self, m):
        ...
        q.tau[m]['b'] = (self.hyper.tau_b[m] + 0.5 * (
                    np.trace(self.X[m]['prodT']) + np.trace(np.dot(q.W[m]['prodT'], q.Z['prodT']))
    ...
    def update_x(self, m):  # Multilabel
        ...
        for n in np.arange(self.n_max):
            # cov
            self.X[m]['cov'][n, :] = (q.tau_mean(m) + 2 * self.lambda_func(q.xi[m][n, :])) ** (
                -1)  # We store only the diagonal of the covariance matrix
            # mean
            self.X[m]['mean'][n, :] = np.dot((self.t[m]['mean'][n, :] - 0.5 + q.tau_mean(m) * (
                        np.dot(q.Z['mean'][n, :], q.W[m]['mean'].T) + q.b[m]['mean'])), np.diag(self.X[m]['cov'][n, :]))
            # prodT
            self.X[m]['prodT'] += np.dot(self.X[m]['mean'][n, np.newaxis].T,
                                         self.X[m]['mean'][n, np.newaxis]) + np.diag(self.X[m]['cov'][n, :])
            # sum(log(det(X)))
        ...
        if self.SS[m]:
            ...
            for i in np.arange(self.d[m] - 1):
                ...
                for j in np.arange(self.d[m] - 1):
                    m_wormk = m_wormj[:, np.arange(
                        self.d[m] - 1) != j]  # it extracts the mean of the values there are neither i nor j
            ...
            for j in np.arange(self.d[m] - 1):
                m_wormk = m_wormj[:,
                          np.arange(self.d[m] - 1) != j]  # it extracts the mean of the values there are neither i nor j
                expj[:, j] = self.expectation_aprx(m_wormi, m_wormk, c=m_wormj[:, j])
            # mean
            self.X[m]['mean'][~t_b] = (m_wormj - (expj.T / xi).T).flatten()
            self.X[m]['mean'][t_b] = m_wormi + np.sum(
                m_wormj - self.X[m]['mean'][~t_b].reshape(self.n_max, self.d[m] - 1), axis=1)
            self.X[m]['prodT'] = np.dot(self.X[m]['mean'].T, self.X[m]['mean'])
    ...
    def predict(self, m_in, m_out, *args):
        ...
        """
        # =============================================================================
        #         Hay que modificarlo para que pueda predecir.
        ...
        if self.method[m_out] == 'reg':
            ...
            # Variance X
            var_x = q.tau_mean(m_out) ** (-1) * np.eye(self.d[m_out]) + np.linalg.multi_dot(
        ...
        # Categorical
            ...
            t_pred_proba = self.cat_nn[m_out].predict_proba(Z=torch.from_numpy(self.Z_mean),
                                                            W=torch.from_numpy(q.W[m_out]['mean']),
                                                            b=torch.from_numpy(q.b[m_out]['mean']))
            return t_pred_proba
    ...
    def update_bound(self):
        ...
        for m in np.arange(self.m):
            ...
            q.tau[m]['ElogpXtau'] = -(0.5 * self.n_max * self.d[m] + self.hyper.tau_a[m] - 1) * np.log(
                q.tau[m]['b'] + sys.float_info.epsilon)
            if self.method[m] == 'mult':  # MultiLabel
                q.tau[m]['ElogpXtau'] += np.sum(
                    np.log(self.sigmoid(q.xi[m])) + self.X[m]['mean'] * self.t[m]['mean'] - 0.5 * (
                                self.X[m]['mean'] + q.xi[m]))
            if self.sparse[m]:  # Even though it sais Walp, it also includes the term related to gamma
                q.alpha[m]['ElogpWalp'] = -(0.5 * self.d[m] + self.hyper.alpha_a[m] - 1) * np.sum(
                    np.log(q.alpha[m]['b'])) - (0.5 * q.Kc + self.hyper.gamma_a[m] - 1) * np.sum(
                    np.log(q.gamma[m]['b']))  # - self.hyper.gamma_b[m]*np.sum(q.gamma_mean(m))
            else:
                q.alpha[m]['ElogpWalp'] = -(0.5 * self.d[m] + self.hyper.alpha_a[m] - 1) * np.sum(
                    np.log(q.alpha[m]['b']))
...
class SparseELBO(nn.Module):
    ...
    def get_params(self):
        ...
        '''
        lengthscale = np.exp(self.kernel.lengthscale_unconstrained.data.cuda().cpu().numpy())
        return lengthscale, self.K.data.cuda().cpu().numpy(), self.kernel.variance.data.cuda().cpu().numpy()
...
class Qdistribution(object):
    ...
    def init_rnd(self, X, method, SS, SS_mask, Z_init=None, W_init=None):
        ...
        Z['prodT'] = Z['cov'] + self.n_max * Z[
            'cov']  # np.dot(self.Z['mean'].T, self.Z['mean']) + self.n_max*self.Z['cov']
        ...
        for m in np.arange(self.m):
            ...
            # Initialization of the matrix W for each view
            W[m]['mean'] = np.random.normal(np.zeros((self.d[m], self.Kc)), 1 / (
                np.repeat(self.alpha_mean(m).reshape(1, self.Kc), self.d[m],
                          axis=0)))  # np.random.normal(0.0, 1.0, self.d[m] * self.Kc).reshape(self.d[m], self.Kc)
            ...
            if method[m] == 'reg' or method[m] == 'mult':
                ...
                self.b[m]['mean'] = self.tau_mean(m) * np.dot(
                    np.sum(self.X[m]['mean'] - np.dot(self.Z['mean'], W[m]['mean'].T), axis=0)[np.newaxis, :],
                    self.b[m]['cov'])
                self.b[m]['prodT'] = np.sum(self.b[m]['mean'] ** 2) + self.d[m] * self.b[m]['cov'][
                    0, 0]  # mean of a noncentral chi-squared distribution
            else:
