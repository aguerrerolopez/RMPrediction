...
#
modelo_a_cargar = "./Results/RyC_5fold0_noamoxi_amika_fosfo_2-12maldi_prun0.01.pkl"
# modelo_a_cargar = "./Results/RyC_noprior_"+familia+"_prun0.01.pkl"


with open("./data/ryc_data_mediansample_noamoxi_noamika_nofosfo_TIC.pkl", 'rb') as pkl:
    ryc_data = pickle.load(pkl)
with open("./data/ryc_5STRATIFIEDfolds_noamoxi_noamika_nofosfo_fullAB.pkl", 'rb') as pkl:
...
familias = [penicilinas, cephalos, monobactams, carbapenems, aminos, fluoro]
...
fig, ax = plt.subplots()
width = 0.15
x = np.arange(len(otros))
rects1 = ax.bar(x - 4*width/2, auc_by_ab[1, :], width, label='Init1')
rects0 = ax.bar(x - 2*width/2, auc_by_ab[0, :], width, label='Init0')
rects2 = ax.bar(x, auc_by_ab[2, :], width, label='Init2')
rects4 = ax.bar(x + 2*width/2, auc_by_ab[4, :], width, label='Init4')
rects3 = ax.bar(x + 4*width/2, auc_by_ab[3, :], width, label='Init3')
ax.axhline(y=0.5, color='r')
ax.set_ylabel('AUC')
ax.set_ylim(bottom=0.2, top=1)
ax.set_title('RyC '+familia+': AUC by AB in fold 0')
ax.set_xticks(x)
ax.set_xticklabels(otros, rotation=30, fontsize='xx-small')
ax.legend()
fig.tight_layout()
