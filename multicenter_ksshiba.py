...
hyper_parameters = {
    "sshiba": {"prune": 1, "myKc": 100, "pruning_crit": 1e-2, "max_it": int(5000)}
}
...
for r in range(5):
    ...
    ## MALDI data.
    x0_tr, x0_val = (
        maldi_greg.loc[folds_greg["train"][r]],
        maldi_greg.loc[folds_greg["val"][r]],
    )
    ...
    # AST
    x1_tr, x1_val = (
        fen_greg.loc[folds_greg["train"][r]],
        fen_greg.loc[folds_greg["val"][r]],
    )
    ...
    # AST
    x1_tr, x1_val = (
        fen_ramon.loc[folds_ramon["train"][r]].dropna(),
        fen_ramon.loc[folds_ramon["val"][r]].dropna(),
    )
    ...
    # View 1: maldis hgugm + nans in RyC data
    maldi_gm_view = np.vstack(
        (x0_greg_train, 0 * x0_ramon_train, x0_greg_test, 0 * x0_ramon_test)
    )

    # view 2: maldis ryc + nans in gm data
    maldi_ryc_view = np.vstack(
        (0 * x0_greg_train, x0_ramon_train, 0 * x0_greg_test, x0_ramon_test)
    )

    # view3: true values in _train and nans in _test
    ast_view = np.vstack((x1_greg_train, x1_ramon_train))

    ##### Declare the data as the model needs it:
    myModel_mul = ksshiba.SSHIBA(
        hyper_parameters["sshiba"]["myKc"], hyper_parameters["sshiba"]["prune"], fs=1
    )
    ...
    if kernel == "pike":
        ...
        for asig in range(maldi_view.shape[0]):
            ...
            print(
                "Preproccessing TOPF " + str(asig) + "/" + str(maldi_view.shape[0]),
                end="\r",
            )
            topf_signal = np.concatenate(
                (
                    np.arange(2000, 12000).reshape(-1, 1),
                    maldi_view[asig, :].reshape(-1, 1),
                ),
                axis=1,
            )
            signal_transformed = transformer.fit_transform(topf_signal)
            x0_new[asig] = MaldiTofSpectrum(
                signal_transformed[signal_transformed[:, 1] > 0]
            )
        maldi_view = [MaldiTofSpectrum(x0_new[i]) for i in range(len(x0_new))]
        maldi_view = myModel_mul.struct_data(
            maldi_view, method="reg", V=maldi_view, kernel="pike"
        )
    elif kernel == "rbf":
        print("Using RBF kernel")
        print("Creating GM view")
        maldi_gm_view = myModel_mul.struct_data(
            maldi_gm_view,
            method="reg",
            V=x0_greg_train,
            kernel="rbf",
        )
        print("Creating RyC view")
        maldi_ryc_view = myModel_mul.struct_data(
            maldi_ryc_view,
            method="reg",
            V=x0_ramon_train,
            kernel="rbf",
        ...
        maldi_gm_view = myModel_mul.struct_data(
            maldi_gm_view, method="reg", V=x0_greg_train, kernel="linear"
        )
        print("Creating RyC view")
        maldi_ryc_view = myModel_mul.struct_data(
            maldi_ryc_view, method="reg", V=x0_ramon_train, kernel="linear"
        )
    else:
        print(
            "You didn't choose a kernel, so we are going to use the MALDI data as raw:"
        )
    ...
    print(
        "################ Results EXPERIMENT Resistant Mechanisms in both hospitals##################"
    )
...
total_number_of_test = 402
weights = np.array(
    [
        85 / total_number_of_test,
        6 / total_number_of_test,
        191 / total_number_of_test,
        9 / total_number_of_test,
        58 / total_number_of_test,
        53 / total_number_of_test,
    ]
)
...
print("Weighted average: " + str(weighted_avg))
...
ord_k = np.argsort(
    myModel_mul.q_dist.alpha[indicator_pos]["a"]
    / myModel_mul.q_dist.alpha[indicator_pos]["b"]
)
# Create the matrix to plot afterwards
matrix_views = np.zeros(
    (len(myModel_mul.q_dist.W), myModel_mul.q_dist.W[0]["mean"].shape[1])
)
# There are 3 views
n_rowstoshow = 3
...
for z in range(n_rowstoshow):
    ...
    if z > 1:
        ...
        matrix_views[z, :] = np.mean(np.abs(myModel_mul.q_dist.W[z]["mean"]), axis=0)[
            ord_k
        ]
...
for v, o in enumerate(order):
    if o == 2:
        ...
        ax[v].set_title(titles_views[o] + " view", color=cmap(v), fontsize=30)
...
samples_end = (
    x0_greg_train.shape[0]
    + np.vstack(ramon_data["full"].loc[folds_ramon["train"][r]]["Hospital"]).shape[0]
)
hospitals = ["GM", "RyC"]
ast = ["ESBL+CP", "ESBL", "WT"]
...
scatter1 = plt.scatter(
    z_tsne_hospital[:, 0][:first_ryc],
    z_tsne_hospital[:, 1][:first_ryc],
    c="tab:red",
    marker="x",
    label="GM domain",
)
...
scatter1 = plt.scatter(
    z_pca_hospital[:, 0][:first_ryc],
    z_pca_hospital[:, 1][:first_ryc],
    c="tab:red",
    marker="x",
    label="GM domain",
)
...
z_tsne_ab = tsne.fit_transform(
    myModel_mul.q_dist.Z["mean"][
        :, np.where(np.abs(myModel_mul.q_dist.W[2]["mean"][0, :]) > 0.01)
    ].squeeze()
)
z_pca_ab = pca.fit_transform(
    myModel_mul.q_dist.Z["mean"][
        :, np.where(np.abs(myModel_mul.q_dist.W[2]["mean"][0, :]) > 0.01)
    ].squeeze()
)
...
ast_label = np.argmax(
    np.vstack((x1_greg_train, x1_ramon_train, x1_greg_test, x1_ramon_test)), axis=1
)
...
scatter1 = ax.scatter(
    z_tsne_ab[:, 0][np.where(ast_label == 0)],
    z_tsne_ab[:, 1][np.where(ast_label == 0)],
    z_tsne_ab[:, 2][np.where(ast_label == 0)],
    c="tab:red",
    marker="x",
    label="ESBL+CP",
)
scatter2 = ax.scatter(
    z_tsne_ab[:, 0][np.where(ast_label == 1)],
    z_tsne_ab[:, 1][np.where(ast_label == 1)],
    z_tsne_ab[:, 2][np.where(ast_label == 1)],
    c="tab:blue",
    marker="o",
    label="ESBL",
)
scatter3 = ax.scatter(
    z_tsne_ab[:, 0][np.where(ast_label == 2)],
    z_tsne_ab[:, 1][np.where(ast_label == 2)],
    z_tsne_ab[:, 2][np.where(ast_label == 2)],
    c="tab:green",
    marker="s",
    label="WT",
)
...
scatter1 = plt.scatter(
    z_pca_ab[:, 0][np.where(ast_label == 0)],
    z_pca_ab[:, 1][np.where(ast_label == 0)],
    z_pca_ab[:, 2][np.where(ast_label == 0)],
    c="tab:red",
    marker="x",
    label="ESBL+CP",
)
scatter1 = plt.scatter(
    z_pca_ab[:, 0][np.where(ast_label == 1)],
    z_pca_ab[:, 1][np.where(ast_label == 1)],
    z_pca_ab[:, 2][np.where(ast_label == 1)],
    c="tab:blue",
    marker="o",
    label="ESBL",
)
scatter1 = plt.scatter(
    z_pca_ab[:, 0][np.where(ast_label == 2)],
    z_pca_ab[:, 1][np.where(ast_label == 2)],
    z_pca_ab[:, 2][np.where(ast_label == 2)],
    c="tab:green",
    marker="s",
    label="WT",
)
